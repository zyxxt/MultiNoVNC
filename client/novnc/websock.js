...
function recv_message(e) {
    ...
    //Util.Warn(">> recv_message: " + e.data.length);
    ...
    try {
        ...
        if (rQlen() > 0) {
            ...
            if (rQ.length > rQmax) {
                ...
                //Util.Warn("Compacting receive queue");
                rQ = rQ.slice(rQi);
                rQi = 0;
            }
        } else {
            Util.Debug("Ignoring empty message");
            Util.Warn("Ignoring empty message");
        }
    } catch (exc) {
...
function init(protocols) {
    ...
    // but it does not.
    try {
        if (bt && ("binaryType"in WebSocket.prototype || 'binaryType' in (new WebSocket("ws://localhost:17523")))) {
