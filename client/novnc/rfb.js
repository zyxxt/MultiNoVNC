...
mouseMove = function(x, y) {
    ...
    Util.Info('>> mouseMove x' + x + ",y" + y + ";absx:" + display.absX(x) + ",vposy:" + display.absX(y));
    mouse_arr = mouse_arr.concat(
            pointerEvent(display.absX(x), display.absY(y)));
...
function sQShift32(data) {
	...
	//Util.Warn("data[0]=" + dataArray[0]+",data[1]=" + dataArray[1] + 
	//	",data[2]=" + dataArray[2] + ",data[3]=" + dataArray[3]);
...
init_msg = function() {
    ...
    var strlen, reason, length, sversion, cversion, repeaterID,
		...
		ery_maj, ery_min, protoc_num, protoc;

	Util.Warn("rfb_state:" + rfb_state);
    ...
    case 'Authentication' :
        ...
        switch (rfb_auth_scheme) {
            ...
            case 2:  // VNC authentication
                ...
                return;
			case 19:
				if (ws.rQwait("VencryptAuth verion", 2)) { return false; }
				ery_maj = ws.rQshift8();
				ery_min = ws.rQshift8();
				//Util.Debug("VencryptAuth verion:" + ery_maj + "." + ery_min);
				if (ery_maj !== 0 || ery_min !== 2) {
					fail("Unsupported encry auth: " + ery_maj + "." + ery_min);
					return;
				}
				Util.Warn("array lenth =" + [0,2].length);
				ws.send([0, 2]);
				updateState('VencryptAuthPlain', "VencryptAuth verion: 0.2 ");
				init_msg();  // Recursive fallthrough (workaround JSLint complaint)
				return;
            default:
        ...
        init_msg();  // Recursive fallthrough (workaround JSLint complaint)
        break;
	case 'VencryptAuthPlain' :
		if (ws.rQwait("VencryptAuth verion result", 1)) { return false; }
		if (ws.rQshift8() != 0) {
			fail("auth failed, verion no match");
			return;
		}
		if (ws.rQwait("number of protocol", 1)) { return false; }
		protoc_num = ws.rQshift8();
		if (protoc_num < 1) {
			fail("get protocol num:" + protoc_num);
			return;
		}

		if (ws.rQwait("protocol", 4 * protoc_num)) { return false; }
		for (i = 0; i < protoc_num; ++i ) {
			if (ws.rQshift32() === 256) { 
				ws.send(sQShift32(256));
				protoc = 256;
			}
		}
		if (protoc !== 256) {
			fail("unsupported sub auth protoc");
			return false;
		}
		updateState('DoAuthPlain', "protoc = 256");
        init_msg();  // Recursive fallthrough (workaround JSLint complaint)
        break;
	case 'DoAuthPlain' :
			ws.send(sQShift32(rfb_user.length));
			ws.send(sQShift32(rfb_password.length));
			ws.send_string(rfb_user);
			ws.send_string(rfb_password);
			updateState('SecurityResult');
            init_msg();  // Recursive fallthrough (workaround JSLint complaint)
			break;
    case 'SecurityResult' :
...
that.sendPassword = function(passwd,user) {
    rfb_password = passwd;
	rfb_user = user;
...
that.testMode = function(override_send, data_mode) {
    ...
    that.connect = function(host, port, password, user) {
            rfb_host = host;
            rfb_port = port;
